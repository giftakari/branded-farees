# coding: utf-8

"""
    Akaris Travels Air

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 11.10.4
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

import openapi_client
from openapi_client.models.signature_on_file import SignatureOnFile  # noqa: E501
from openapi_client.rest import ApiException

class TestSignatureOnFile(unittest.TestCase):
    """SignatureOnFile unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test SignatureOnFile
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SignatureOnFile`
        """
        model = openapi_client.models.signature_on_file.SignatureOnFile()  # noqa: E501
        if include_optional :
            return SignatureOnFile(
                type = 'SignatureOnFile', 
                date_effective_expire = openapi_client.models.date_effective_expire.DateEffectiveExpire(
                    effective = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(), 
                    expire = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(), 
                    expire_date_exclusive_ind = True, ), 
                signature_on_file_ind = True
            )
        else :
            return SignatureOnFile(
        )
        """

    def testSignatureOnFile(self):
        """Test SignatureOnFile"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
