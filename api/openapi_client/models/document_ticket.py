...
class DocumentTicket(Document):
    """
    DocumentTicket
    """
    control_number: constr(strict=True, max_length=32) = Field(..., alias="ControlNumber", description="The control number assigned to the Ticket")
    historic_ind: Optional[StrictBool] = Field(None, alias="historicInd", description="If true, this document has been superseded by a new Document facet and may have been exchanged, voided or refunded")
    __properties = ["@type", "Number", "TravelerIdentifierRef", "Amount", "WaiverCode", "Commission", "CumulativeValue", "IssuingPCC", "IssuingIATA", "IssuingCity", "FiledAmount", "ControlNumber", "historicInd"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DocumentTicket:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DocumentTicket:
        """Create an instance of DocumentTicket from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DocumentTicket.parse_obj(obj)

        _obj = DocumentTicket.parse_obj({
            "type": obj.get("@type"),
