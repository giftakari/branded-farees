...
class BrandTextDetail(BrandText):
    """
    BrandTextDetail
    """
    sequence: Optional[StrictInt] = Field(None, description="The order of the text block, if there are more than one block.")
    description: Optional[constr(strict=True, max_length=1024)] = Field(None, description="Assigned Type: c-1100:Description")
    ...
    __properties = ["@type", "id", "target", "TextFormatted", "sequence", "description", "Image", "URL", "DateCreateModify"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> BrandTextDetail:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> BrandTextDetail:
        """Create an instance of BrandTextDetail from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return BrandTextDetail.parse_obj(obj)

        _obj = BrandTextDetail.parse_obj({
            "type": obj.get("@type"),
            "id": obj.get("id"),
